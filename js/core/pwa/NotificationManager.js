...
(function (global) {
  ...
  onAppReady(function () {
    ...
    // PHASE N.4 — boot-time FCM token refresh (PHASE N.1's confirmed
    // gap: registerFcmTokenIfAvailable() previously ran ONLY from the
    // two explicit-gesture call sites above — handleEnableNotifications
    // Click()/handleRegisterFcmDeviceClick() — never automatically on a
    // later normal boot. A silently rotated/renewed FCM token therefore
    // stayed unknown to the server until the person happened to revisit
    // Settings and re-trigger one of those buttons.) Fire-and-forget,
    // non-blocking (not awaited — onAppReady()'s caller, BootManager,
    // does not wait for this callback either), non-prompting (only
    // permissionState()==='granted' — never 'default' — so this never
    // shows a permission dialog), and idempotent (registerFcmToken
    // IfAvailable()'s own ApiService.saveData() call upserts by
    // id===token — re-registering an unchanged token just refreshes
    // updated_at on the same row, never creates a duplicate; see that
    // function's own doc comment). Uses the EXISTING, unmodified
    // registerFcmTokenIfAvailable() — no new registration logic, no new
    // FCM client, no أجهزة_FCM schema change.
    safely(function () {
