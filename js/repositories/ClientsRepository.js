...
(function (root) {
  ...
  /**
   * الحقول الموسّعة الجديدة لبيانات الموكل (شخص طبيعي/اعتباري + الحقول
   * المتكررة JSON) — أُضيفت بنفس منطق CLIENTS_LEGACY_FIELDS أعلاه، لكن
   * كمصفوفة منفصلة إضافية بدل تعديل الأصلية، حفاظًا على توثيق الملف
   * القائم ("100% Behavior Compatible" مع السلوك القديم). تُستخدم أدناه
   * فقط لتوسعة نطاق البحث النصي الحر ليشمل بيانات الشركة/الهواتف/
   * العناوين/التوكيلات المتعددة أيضًا.
   */
  var CLIENTS_EXTENDED_FIELDS = [
    'الجنسية', 'رقم_جواز_السفر', 'تاريخ_الميلاد', 'الجنس',
    'نوع_الشركة', 'نشاط_الشركة', 'القانون_التابع_له', 'رقم_السجل',
    'تاريخ_بداية_السجل', 'تاريخ_نهاية_السجل', 'الرقم_الموحد', 'الرقم_الضريبي',
    'أرقام_الهواتف', 'العناوين', 'التوكيلات', 'المستندات', 'المديرين',
    'حساب_مفعل', 'اسم_المستخدم'
  ];
  ...
  ClientsRepository.prototype._matchesSearch = function (record, term) {
    ...
    var joined = CLIENTS_LEGACY_FIELDS.concat(CLIENTS_EXTENDED_FIELDS)
