...
//
// SPLASH RACE FIX (PHASE A9): this listener used to call
// hideSplashAndCheckFirstRun() immediately, with no relation to
// MIN_VISIBLE_MS. On a fast device/fast boot, 'application:ready' can
// fire well under MIN_VISIBLE_MS after window.__splashStart, so this
// was the one call site that could cut the staged CSS reveal timeline
// (logo -> title -> subtitle -> version -> contact -> footer) short —
// _beginSplashFadeOut() is only guarded against firing twice
// (__splashHideStarted), not against firing too early. It now waits
// out whatever is left of MIN_VISIBLE_MS first, exactly like the
// DOMContentLoaded listener below, so readiness can speed up the
// splash's exit but never truncate the animation (requirement: "if
// init finishes before the animation, wait for the animation"). If
// content is already ready AFTER MIN_VISIBLE_MS has elapsed (the slow
// boot case), 'remaining' is 0 and this fires on the next tick, same
// as before — no added delay in that case.
...
async function wizardSaveAndStart() {
  ...
  // PHASE A7.5 — SyncCoordinator: was setTimeout(loadFromSheets,500).
  // Reason is 'boot', not 'manual': this is the very first connection
  // this device has ever made (no prior success, no prior sync attempt
  // yet recorded), so shouldSync('boot') already returns true on its
  // own — no TTL/cooldown bypass is actually needed to preserve the
  // current "sync immediately after first connecting" behavior.
  setTimeout(function () {
    if (typeof SyncCoordinator !== 'undefined') {
      SyncCoordinator.requestSync('boot');
    } else if (typeof loadFromSheets === 'function') {
      loadFromSheets();
    }
  }, 500);
  closeFirstRunWizard();
