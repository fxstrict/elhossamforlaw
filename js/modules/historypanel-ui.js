...
(function () {
  ...
  function openHistoryPanel() {
    ...
    // PHASE UX-BUGFIX (history panel) — the app already has a dedicated,
    // reference-counted background-scroll lock built exactly for this
    // symptom (js/core/modal/ScrollLockManager.js, "the page moves/bleeds
    // through behind the overlay"), used by every js/core/modal/* dialog.
    // The history panel never called it, so the dashboard underneath
    // stayed scrollable while this fixed/translucent-shadowed panel slid
    // over it — on Android Chrome that produces exactly the torn/"ghosted"
    // double-image look (sidebar badges/menu bleeding through the lower
    // half of the panel) reported after the previous fix. Guarded by
    // `wasOpen` so calling openHistoryPanel() twice in a row can't lock
    // twice without a matching close in between.
    if (!wasOpen && window.ModalScrollLockManager) window.ModalScrollLockManager.lock();
    renderHistoryPanel();
    startRelTimeTicker();
  ...
  function closeHistoryPanel() {
    ...
    // PHASE UX-BUGFIX (history panel) — release the shared scroll lock
    // acquired in openHistoryPanel(), matching the same lock/unlock
    // pairing every js/core/modal/* dialog already uses. Guarded by
    // `wasOpen` so a stray/duplicate close call can't unlock more times
    // than the panel actually locked.
    if (wasOpen && window.ModalScrollLockManager) window.ModalScrollLockManager.unlock();
    stopRelTimeTicker();
