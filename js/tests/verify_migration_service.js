...
async function main() {
  ...
  await checkAsync('[Empty database] migrate() completes (not skipped) with zero records moved', async () => {
    ...
    assert.strictEqual(report.entities.length, 18); // PHASE 31 (RBAC): schema grew 12->15 stores (11->14 excluding metadata). PHASE 37 (Opponents): 15->16 stores (14->15 excluding metadata). Later phases (processServerWorks, caseClients, expenses): 16->19 stores (15->18 excluding metadata).
  ...
  await checkAsync('[Empty database] checkpoint status is "completed" and getStatus() reflects it', async () => {
    ...
    assert.strictEqual(status.completedEntities.length, 18); // PHASE 31 (RBAC): 11 -> 14. PHASE 37 (Opponents): 14 -> 15. Later phases: 15 -> 18.
  ...
  await checkAsync('[Double migration] force:true re-runs every entity from scratch and still ends in the same state', async () => {
    ...
    assert.strictEqual(forced.entities.length, 18); // PHASE 31 (RBAC): 11 -> 14. PHASE 37 (Opponents): 14 -> 15. Later phases (processServerWorks, caseClients, expenses): 15 -> 18.
    const clientsOut = await target.read('clients');
  ...
  check('MigrationService defaults entityKeys to every schema store except "metadata"', () => {
    ...
    assert.strictEqual(svc._entityKeys.length, 18); // PHASE 31 (RBAC): 11 -> 14 (adds 'users', 'auditLog', 'loginLog'). PHASE 37 (Opponents): 14 -> 15 (adds 'opponents'). Later phases: 15 -> 18 (adds 'processServerWorks', 'caseClients', 'expenses').
  });
